...
provide the latest stable releases but you can pull a specific release, 
...
You can check https://github.com/hazelcast/hazelcast-kubernetes[Hazelcast Kubernetes] 
...
discover other members, to do so create a file named (`hazelcast-jet-config.yaml`) 
...
Hazelcast Jet jobs to your cluster, see the https://github.com/hazelcast/hazelcast-jet-code-samples/tree/master/integration/kubernetes[Kubernetes Integration] 
code sample
...
To deploy Hazelcast Jet using Helm, you need to add the Hazelcast Helm 
Charts repository to Helm using the following command:

```bash
helm repo add hazelcast https://hazelcast.github.io/charts/
``` 

Then, update information of available charts locally from chart 
...
the standard `kubectl scale` (See https://kubernetes.io/docs/tasks/run-application/scale-stateful-set/#scaling-statefulsets[Scaling StatefulSets] 
for more information.) command to change the cluster size.
...
With the https://docs.hazelcast.org/docs/jet/latest-dev/manual/#automatic-elasticity[Automatic Elasticity] 
feature, your jobs will be automatically restarted from the latest known
snapshot when you downscale your cluster. For more detauls about 
snapshotting and processing guarantees see https://docs.hazelcast.org/docs/jet/latest-dev/manual/#fault-tolerance[Fault Tolerance] 
section on the https://docs.hazelcast.org/docs/jet/latest-dev/manual/[Hazelcast Jet Reference Manual].
