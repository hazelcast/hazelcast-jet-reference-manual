...
image to start the Hazelcast Jet Management Center or a Hazelcast Jet 
...
If you want to start a customized Hazelcast Jet instance, you can extend 
the Hazelcast Jet image by providing your own configuration file.
...
deploy into Kubernetes environments. 

Hazelcast Jet Kubernetes-ready Docker images uses https://github.com/hazelcast/hazelcast-kubernetes[Hazelcast Kubernetes]
plugin to discover other Hazelcast Jet members in the Kubernetes environment 
by interacting with the Kubernetes API. Using Kubernetes API requires granting 
certain permissions. Therefore, you may need to create Role Based Access
Control definition, `rbac.yaml`, with the following content. 
...
The required permissions above can be granted with the following command: 

```bash
kubectl apply -f rbac.yaml
```

Then Hazelcast Jet needs to be configured to use Kubernetes Discovery 
to discover others members.
...
and this object holds our Kubernetes Discovery enabled Hazelcast Jet configuration. 
We will use this configuration when we are creating our pods in the following section. 

```bash
kubectl apply -f hazelcast-jet-config.yaml
```

We will create a StatefulSet, which manages Pods that are based on an identical 
container spec, and our Service, which is an abstraction which defines a logical 
set of Pods and a policy by which to access them, with the following content.
...
After that you can inspect pod logs to see that they formed a cluster with the following command:
...
For more detailed deployment information and how to submit Hazelcast 
Jet jobs to your cluster, check out the https://github.com/hazelcast/hazelcast-jet-code-samples/tree/master/integration/kubernetes[Kubernetes Integration] 
code sample.

== Deployment using Helm

Hazelcast Jet provides Helm charts which is an easy way to package 
and deploy applications on Kubernetes. 

In order to deploy Hazelcast Jet with Helm, the Hazelcast Helm Charts 
repository need to be added to Helm with the following command :
...
Then, you can install the chart by:

```bash
helm install hazelcast/hazelcast-jet
``` 

Check out https://github.com/hazelcast/charts/tree/master/stable/hazelcast-jet[Hazelcast Jet Helm Chart]
page for more detailed configuration and https://github.com/hazelcast/charts#troubleshooting-in-kubernetes-environments[Troubleshooting in Kubernetes Environments] 
