...
places a higher degree of responsibility on the developer. As a rule of
thumb, follow these rules to avoid concurrency bugs:

* don't use an item after emitting it
* don't mutate an item you received

However, working with immutable objects has its price, especially when
they are large. You spend CPU time on copying the data and you create
a lot of GC pressure from short-lived objects. This is why we also
present fine-grained rules you _must_ follow to stay safe while sharing
the data between pipeline stages.
...
Its contract states that the result won't be modified after emitting,
unlike the
{jet-javadoc}/aggregate/AggregateOperation.html#finishFn--[`finishFn`],
which Jet uses only when it knows the whole operation is done and it
will never touch the aggregated data again.

**2. _Either_ keep using the item you emitted (read-only), _or_ mutate
it downstream**
...
**3. If you create a fork in the pipeline by attaching several
downstream stages, only one of them may mutate the item.**

In this example there are two downstream `map` stages that both mutate
the same object they get from the upstream. This results in concurrent
modification and causes erratic bugs:
...
<1> You modify the person object in one mapping stage
<2> You modify it as well in the other mapping stage
