...
Hot Restart Persistence works by continuously persisting the RAM-based
snapshot data. Even with the fastest solid-state storage, this
dramatically reduces the maximum snapshotting throughput available to
Jet: while DRAM can take up to 50 GB/s, a high-end storage device caps
at 2 GB/s. Keep in mind that this is throughput per member, so even on a
minimal cluster of 3 machines, this is actually 6 GB/s available to Jet.

Tu use Lossless Cluster Restart, you have to enable it in
`hazelcast-jet.xml`:
...
In addition to that, you have to configure Hot Restart in
{hz-refman}#global-hot-restart-configuration[IMDG configuration]:
...
When you start the cluster again, it must consist of exactly the same
members as before the shutdown. Beyond that, there's no special action
to take: as soon as the cluster re-forms, it will automatically reload
the persisted snapshots and resume the jobs.
