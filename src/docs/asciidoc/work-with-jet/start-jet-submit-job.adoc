...
`Distributed` prepended. For example, a `FunctionEx` is just
...
== Cancelling the Job, Exporting its State and Updating the Pipeline

If you want to terminate the job before it completes or to terminate a
streaming job which normally never completes, use:

[source]
----
include::{javasource}/ManageJob.java[tag=s7]
----
...
You can also export the state and keep the job running:
...
Then you can use the exported state to submit a new job that will use the
...
*Note:* exporting of the snapshot is only available in Jet Enterprise.
