...
for bugfixing or to improve the application.
...
Job Upgrades use the snapshots. Job state is exported to the snapshot 
and the new job version starts from from the snapshot.
...
old and new version will run in parallel. This is useful e.g. 
for an A/B testing.
...
also use exported snapshot as a point of recovery.
