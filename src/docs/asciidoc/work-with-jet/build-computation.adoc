...
(simple examples: sum and average) over the data items. Typically you
...
an infinite event stream, you must define a finite _window_ over the
...
We simply counted all the lines in a Hazelcast list named "text" and
pushed the count to a list named "result". Let's count all the words
instead:
...
there are two such transforms of special interest: co-aggregation and
hash-join. We discuss these next.

[[cogroup]]
=== Co-Aggregation

Just as we performed aggregate and group-and-aggregate operations above,
we can peform co-aggregate and cogroup-and-aggregate of several stages
together. In SQL terms this is analogous to a JOIN coupled with a GROUP
BY. The JOIN condition is constrained to matching on the grouping key.
...
making the stream-1 words weight ten times more than those in stream-0:
