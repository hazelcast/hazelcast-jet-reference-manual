...
----
include::{sourceDir}/BuildComputation.java[tag=simple]
----
...
StageWithGrouping<Object, Object> src1 = p.drawFrom(Sources.list("src1"))
                                          .groupingKey(wholeItem());
StageWithGrouping<Object, Object> src2 = p.drawFrom(Sources.list("src2"))
                                          .groupingKey(wholeItem());
src1.aggregate3(src2, counting2())
    .drainTo(Sinks.map("result"));

For further details on co-grouping please refer to the <<cogroup,
...
This is how a very simples batch aggregation without grouping may look:
