...
. `mapUsingX` transforms enrich a stream from a dataset external to the
Jet job. There's direct support for <<map-using-imap, Hazelcast maps>>
and Jet <<map-using-context, exposes the underlying machinery>> as well
so you can write your own code to join with an arbitrary external
dataset.

The hash-join is ideal for batch computations because it ingests all the
enriching data on all members of the computing cluster, so lookup is
both data-local and thread-local -- the fastest you can get.

When you enrich directly from an `IMap` or other storage system, you
get the advantage that the enriching dataset can change during a job.
This is especially relevant to infinite stream processing jobs where
you want to keep updating the enriching dataset as the job runs. You pay
for that with slower access times. Also keep in mind that, if you run a
fault-tolerant job and experience a restart, you may get different
enriching data for the items you are processing again after resuming
