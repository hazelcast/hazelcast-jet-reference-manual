...
items. You can refer to their Javadoc for finer detail and we'll move on
...
Joining two streams means finding all items in both streams that match
according to some rule. These are your joining tools in Jet:

. <<hash-join, _Hash-join_>> is a many-to-one join that matches a
foreign key in one stream with the primary key in another. It can join
several streams in one step. Its main use case is enriching one stream
with the data from one or more additional streams.

. `xUsingY` (such as <<map-using-imap, `mapUsingIMap`>> or <<map-using-context,
`filterUsingContext`>>) enrich a stream from a dataset external to the Jet job. There's direct support
for <<map-using-imap, Hazelcast maps>> and Jet <<map-using-context,
...
perform aggregation of the groups of items with the same key. This is
the most general joining facility.

Hash-join's strength is speed. If you're doing data enrichment and your
enriching dataset doesn't change for the duration of the Jet job, this
is your best tool.

If you're enriching an infinite stream and the dataset evolves over time,
you can apply one of the `xUsingY` transforms to look up directly from
the source. If your source is a Hazelcast map inside the Jet cluster,
you can achieve data locality and excellent throughput. Be sure to let
Jet extract the lookup key by calling `stage.groupingKey()` before
applying the transform. Jet will then be able to partition the stream
by your lookup key, which is essential for data locality.

If you're looking up data from an external system (for example, a
Hazelcast IMDG cluster), data locality is not an option, but you can
still use caching (for example, the Hazelcast client has the _Near Cache_
feature). You should specify the `groupingKey()` so that your cache will
be partitioned and not duplicated on each Jet processor.
...
expected. However, the output may also behave erratically. Say you
change the enriching data for some key from A to B. You expect the
output to show a sequence ...`A-A-B-B`..., but actually you can observe
`A-B-A-B` or even `B-B-A-A`. This is because Jet doesn't enforce the
processing order to strictly follow the time order of events. Currently
there is no feature in Jet that would achieve sequential consistency
while enriching an event stream from a changing dataset.

Cogroup-and-aggregate is the most powerful joining facility in Jet. You
can join any number of streams on a common key. The key can be anything
you can calculate from the data item. In the same step you can apply an
aggregate function to all the grouped items, separated by the
...
stream. You can perform several such joins in one operation, enriching
...
If you use a Hazelcast map that is stored inside the Jet cluster, you
can achieve data locality. For `ReplicatedMap` that's trivial because
...
data. Here, too, you can achieve data locality by letting Jet send your
data item to the member that holds that particular item's lookup key.
This still means the data travels over the network, but it's only one
way and highly streamlined. There's no latency inherent to a
request-response cycle, just the bandwidth cost of network transfer.
...
stages together. In SQL terms this is analogous to a `JOIN` coupled with
a `GROUP BY`. The `JOIN` condition is constrained to matching on the
grouping key.
