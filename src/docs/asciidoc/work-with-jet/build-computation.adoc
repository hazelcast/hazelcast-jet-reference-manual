...
You can achieve various types of join with Jet. These are your basic
tools:

* <<hash-join, _hash-join_>> is a many-to-one join that matches a
foreign key in one stream with the primary key in another.
* <<map-using-context, `map/flatMapUsingContext`>> allow you to join
the stream data with the data in a Hazelcast `ReplicatedMap`, `IMap` or
any other structure, including non-Hazelcast ones.
...
[[map-using-contex]]
== Map/Filter/FlatMap using a Context Object

You can assign a context object to each processor in the cluster that
executes a transforming step in the pipeline. One interesting way to use
this is getting access to a Hazelcast `IMap` or a similar structure so
you can use its data to transform the stream items.

You can't just fetch a `ReplicatedMap` or `IMap` instance and add it to
the pipeline. Instead you provide a _factory_ that Jet will use to
create the context object for each processor in the cluster that
executes your transforming step.

In this example we enrich a stream from an `ReplicatedMap`:
...
Finally, if you're processing an unbounded event stream, you must define
a bounded _window_ over the stream within which Jet will aggregate the
data.
