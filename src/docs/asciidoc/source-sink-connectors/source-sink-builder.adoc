...
Jet will repeatedly call the `fillBufferFn` whenever it needs more data
items. Optimally, the function will fill the buffer with the items it
can acquire without blocking. A hundred items at a time is enough to
...
We have presented an example of a distributed sequence generator. If
there are 2 members there will be 4 parallel generators. The 1st will
generate numbers 0, 4, 8..., the second will generate 1, 5, 9... etc.

In the real world there are some fundamental differences. The data
source is partitioned in advance and you cannot request the number of
partitions that perfectly matches your topology. Instead you must write
...
=== Fault-Tolerant Source

To add fault tolerance to your source, call `createSnapshotFn` and
`restoreSnapshotFn` to save and restore the state.
...
The source above generates an infinite sequence of integers and is
fault-tolerant. It saves the current value to the snapshot and restores
it, if the job is restarted, to correctly continue from the saved value.

Note that the `restoreSnapshotFn` receives a `List` of state objects.
The reason for this is that if the source is distributed, all parallel
instances receive all saved state objects, they need to extract from
them the part that pertains to each parallel instance. In the case of
non-distributed source the list contains exactly one element.

You can add fault tolerance only to streaming sources, batch sources
don't support it.
