...
API (j.u.s for short), you can use Jet's implementation of it to access
a subset of features in Jet. J.u.s allows you to create simple
transform-and-aggregate jobs on bounded data.

To leverage the full power of Jet, use the <<pipeline-api, Pipeline
API>>. Among other things, with it you can use several sources and sinks
in the same job, perform joins, and process unbounded event streams.
You'll also avoid the mismatches between the j.u.s paradigm and the
basic facts of distributed computing.

A j.u.s computation job starts when you invoke a terminal operation such
