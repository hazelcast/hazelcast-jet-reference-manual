...
https://docs.hazelcast.org/docs/latest/manual/html-single/index.html#mutual-authentication[Mutual Authentication]
...
See https://docs.hazelcast.org/docs/jet/latest-dev/javadoc/com/hazelcast/jet/config/MetricsConfig.html[Javadoc]
for more information.

2+| You can also use Hazelcast Jet Management Center to monitor your Jet clusters. See
its ???link???[documentation] for more information.
|===
...
- `JetConfig cfg = new XmlJetConfigBuilder(inputStream).build();`

- `JetConfig cfg = new ClasspathXmlJetConfig(xmlFileName);`

- `JetConfig cfg = new FileSystemXmlJetConfig(cnfigFilename);`

- `JetConfig cfg = new UrlXmlJetConfig(url);`

- `JetConfig cfg = new InMemoryXmlJetConfig(xml);`

Building JetConfig from the YAML declarative JetConfiguration:

- `JetConfig cfg = new YamlJetConfigBuilder(inputStream).build();`

- `JetConfig cfg = new ClasspathYamlJetConfig(yamlFileName);`

- `JetConfig cfg = new FileSystemYamlJetConfig(configFilename);`

- `JetConfig cfg = new UrlYamlJetConfig(url);`

- `JetConfig cfg = new InMemoryYamlJetConfig(yaml);`
...
Declarative configurations (either XML or YAML) has support for 
...
different declarative configuration files. Composing configuration files
is supported both in XML and YAML configurations.
...
NOTE: Use the `<import/>` element on top level of the XML hierarchy.

NOTE: Use the `import` mapping on top level of the YAML hierarchy.

Hazelcast Jet and Hazelcast IMDG share same syntax for declarative
configuration composition. For more examples on importing resources from
the classpath and file system or resources with variables in their 
names, refer to the {hz-refman}#composing-declarative-configuration[relevant section] 
of Hazelcast IMDG reference manual.
...
property programmatically or you use the command line interface. You can
...
Hazelcast Jet and Hazelcast IMDG share same syntax for variable
replacers. You can configure your variable replacers in Hazelcast Jet 
configuration via XML or YAML. Please see {hz-refman}#variable-replacers[Variable Replacers]
section for more information and examples about variable replacers on 
