...
NOTE: The YAML files needs to have a `.yaml` or `.yml` extension if
...
Hazelcast Jet and Hazelcast IMDG shares the same syntax for declarative
...
configuration via XML or YAML. Please see {hz-refman}#variable-replacers[Variable Replacers section]
