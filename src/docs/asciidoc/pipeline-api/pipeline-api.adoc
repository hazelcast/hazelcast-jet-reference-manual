...
there is a risk of high event time skew occuring across partitions as
...
To address the limitation, Jet provides the `apply()` method on all
stages of the pipeline. It allows you to extract any part of the
pipeline into a method and the use that method when building your
pipeline. For example you can extract multiple mapping and filtering
steps, source declaration along with initial enrichment, saving to audit
tables etc.
