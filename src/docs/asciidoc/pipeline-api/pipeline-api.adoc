...
items, effectively avoiding actual aggregation. You can achieve any kind
of join: inner/outer/full, left and right. The constraint, as already
noted, is that the join condition cannot be fully general, but must
amount to matching a computed key.
...
then do the maths on them at the end:
...
input stage in isolation. In some advanced use cases you may have to
perform the "blending" of all items even in the accumulation phase. In
that case you can supply your custom two-input aggregate operation.

== Joining Without Aggregating
...
[source]
----
BatchStage<Tuple2<PageVisit, AddToCart>> joined =
    pageVisits.join(addToCarts);
----
...
If it's also mandatory that you get a stream of all the combinations
of matched items, you can add a flatmapping stage:
