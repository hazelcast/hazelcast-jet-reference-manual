...
enrichment. It is like a one-to-many or many-to-many SQL JOIN that
matches a foreign key in the stream-to-be-enriched with a key in the
enriching dataset. You can perform several such joins in one operation,
enriching your stream from arbitrarily many sources.
