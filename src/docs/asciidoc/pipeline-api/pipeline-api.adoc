...
(`DistributedFunctions.wholeItem()` returns the identity function). Now
our pipeline produces not one, but as many distinct result items as
there are words in the text. What we've just built up is the classical
Word Count task.
...
window's end, so the latency will typically be five seconds or more.
...
such as `flatMap` and `filter`, propagating the timestamp value.
