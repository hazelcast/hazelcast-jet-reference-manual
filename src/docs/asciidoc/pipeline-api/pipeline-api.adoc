...
according to some rule and performing a computation over the matches.
These are your joining tools in Jet:
...
with the data from one or more additional streams. The enriching streams
must be finite (i.e., a `BatchStage`).
...
is your best tool. If you're enriching an infinite stream and want to
observe updates to the enriching dataset, consider enriching your data
by <<enrich-by-lookup, direct lookup from the source>> instead of
streaming in the dataset.
...
[[enrich-by-lookup]]
= Enrich Your Data Using Direct Lookup
...
If your source is a Hazelcast map inside the Jet cluster, you can
achieve data locality and excellent throughput. If you are using an
`IMap`, be sure to let Jet extract the lookup key by calling
`stage.groupingKey()` before applying the transform. Jet will then be
able to partition the stream by your lookup key, which is essential for
data locality. If you use a `ReplicatedMap`, there's no need for that
because its entire contents are replicated on each machine in the
cluster.
...
data. You must let Jet send your data item to the member that holds that
particular item's lookup key. This still means the data travels over the
...
The info is stored in a Hazelcast `IMap` so we use `groupingKey()` to
let Jet partition our stream and use local `IMap` lookups:
