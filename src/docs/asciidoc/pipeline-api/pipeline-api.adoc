...
If you're doing many-to-many enrichment, be extra careful to avoid
accidental data races. You are likely to do enrichment by setting the
enriching item as a property of the main item:
...
times while missing the other ones, and even completely broken product
objects (partially initialized).
...
= Pipeline API Extensibility

The Pipeline API, due to limitations of the Java language, doesn't let
you add methods to the various stages. You are limited to the methods
provided out of the box.

To address the limitation, Jet provides the `apply()`. It allows you to
extract a part of the pipeline into a method and the use that method
when building your pipeline. For example you can extract multiple
mapping and filtering steps, entire windowing aggregations etc. and then
apply the extracted part to a pipeline in a fluent style.
