...
provides higher throughput thanks to making more parallel calls possible
without blocking a thread

- synchronous: the functions return the result directly. Use it if your
external service doesn't provide async api. It's also useful if the
context is for example a pre-loaded machine learning model which is only
cpu-bound and doesn't do any IO.

You can also specify a function that extracts a key from your items
using
{jet-javadoc}/pipeline/GeneralStage.html#groupingKey-com.hazelcast.jet.function.DistributedFunction-[`groupingKey()`].
