...
factory of `ProcessorSupplier` s. `DistributedSupplier<Processor>`
...
The **global parallelism** of a vertex is the total number of its
...
with watermarks: the <<max-watermark-retention, event skew>> grows
without bound.
