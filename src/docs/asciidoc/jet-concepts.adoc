...
At the heart of Jet is the :jet-core:[`TaskletExecutionService`].
...
events `E1` and `E2` in any source partition where `E2` occurs after
`E1` and `E1.timestamp - allowedLag > E2.timestamp`.
...
<<not-adding-timestamps-in-source>>. Also Map Journal and Cache Journal
sources currently don't coalesce watermarks in source, they fall into
this category too.
...
- There are other watermark policies in Core API (not available in
Pipeline API) which can advance the watermark in the absence of events.
See their javadoc.
