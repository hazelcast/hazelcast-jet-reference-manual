...
== Updating the DAG without Losing the State
...
==== Recombining Frames of Sliding Windows
...
[[non-unique-name-update]]
==== Updating jobs with auto-assigned stage names

The Pipeline API allows you to set the name using
{jet-javadoc}/pipeline/Stage.html#setName-java.lang.String-[`Stage.setName`],
but if one is not specified, a default name is used. If multiple stages
have the same name, Jet appends `-N` to the name, where the `N` is a
sequence number. For example, if you have 3 sliding window aggregations
in the job, their names will be `sliding-window`, `sliding-window-2` and
`sliding-window-3`. The sequence number is added in the order they are
added to the `Pipeline`.

Since the vertex name is an identifier of the state in a state snapshot,
it's very important to keep it equal with previous job version. For
example, you can't reorder the aggregations or remove the first one,
because the names will change and incorrect state will be restored to
the processors. Therefore we recommend that you explicitly assign a
unique name in more complex pipelines.

==== Changing of the grouping key

You can change the `groupingKey`'s key extractor. But keep in mind that
the old state will be restored with keys extracted using the previous
key extractor and if the new keys aren't equal, they will create new
groups.

You can also change the key type. In this case, however, runtime
`ClassCastException` can occur because the downstream might expect
another type than it will actually receive. For example, if you change
the key type from `Integer` to `Long`, the downstream stage of a window
aggregation will expect `TimestampedEntry<Long, V>`, but can actually
`TimestampedEntry<Integer, V>`.
