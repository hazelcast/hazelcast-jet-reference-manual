...
* When a processor receives a watermark, it should add it to the outbox
  after handling it. The watermark should be emitted after all items
  that came before it were processed and results based on them emitted.
  Sink processors don't have to emit watermarks.
